...
from career_api_provider import CareerAPIError, recommend_careers_via_api
...
def _run_fallback_workflow(
    *,
    ...
    return {
        "status": "success",
        "engine": "fallback",
        "career_data_source": "local_dataset",
        "user_skills": user_skills,
...
def _run_api_workflow(
    *,
    ...
    api_result = recommend_careers_via_api(
        user_skills=user_skills,
        interests=interests,
        career_goal=career_goal,
        top_n=3,
    ...
    learning_plan = generate_learning_plan(api_result.get("missing_skills", []))
    top_role = api_result["recommended_roles"][0]["role"] if api_result.get("recommended_roles") else "Career Path"
    interview_questions = generate_interview_questions(top_role)
    interview_questions["role"] = top_role
    ...
    return {
        "status": "success",
        "engine": "career_api",
        "career_data_source": api_result.get("provider", "career_api"),
        "user_skills": user_skills,
        "recommended_roles": api_result.get("recommended_roles", []),
        "missing_skills": api_result.get("missing_skills", []),
        "career_intel": api_result.get("career_intel", []),
        "learning_plan": learning_plan,
        "interview_questions": interview_questions,
        "education": education,
        "interests": interests,
...
def run_workflow(user_input: dict) -> dict:
    ...
    if os.environ.get("ONET_API_KEY", "").strip():
        try:
            return _run_api_workflow(
                user_skills=user_skills,
                interests=interests,
                education=education,
                career_goal=career_goal,
            )
        except CareerAPIError:
            # Fall back to the local stack if the career API is unavailable.
            pass
